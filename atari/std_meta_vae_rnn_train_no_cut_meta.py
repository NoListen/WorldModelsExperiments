...
def build_rnn_with_vae(vae, rnn, rnn_lv_dict, comp, z_size, seq_len, batch_size, rnn_lr, kl_tolerance=0.5):
    rnn_loss, mean, logstd = rnn_with_vae(vae, rnn, comp.x, comp.z, comp.a, rnn_lv_dict,
    ...
    rnn_meta_loss, meta_mean, meta_logstd = rnn_with_vae(vae, rnn, comp.mx, comp.mz, comp.ma, rnn_lv_dict,
                                       z_size, batch_size, seq_len, kl_tolerance)
    ...
    return rnn_loss, rnn_meta_loss, logstd, meta_logstd, kl2vae, meta_kl2vae
...
def learn(sess, n_tasks, z_size, data_dir, num_steps, max_seq_len,
          ...
          meta_lr=0.001, min_meta_lr=0.00001, meta_decay=0.998,
          vae_dir="tf_vae",
    ...
    tf_meta_lr = tf.placeholder(tf.float32, shape=[])
    ...
    for i in range(n_tasks):
        ...
        rnn_loss, rnn_meta_loss, rnn_logstd, rnn_meta_logstd, kl2vae, meta_kl2vae = build_rnn_with_vae(vae, rnn, 
                                               tmp_rnn_lv_dict, comp, z_size,
                                               seq_len, batch_size_per_task, tf_meta_lr, kl_tolerance)
        ...
        kl2vaes.append(kl2vae)
        meta_kl2vaes.append(meta_kl2vae)
    ...
    vae_meta_opt = tf.train.AdamOptimizer(tf_lr, name="meta_vae_opt")
    ...
    for i in range(joint_num_steps):
        ...
        step = sess.run(global_step)
        curr_lr = (curr_lr - min_lr) * decay + min_lr
        curr_meta_lr = (curr_meta_lr - min_meta_lr) * meta_decay + min_meta_lr
        ...
        feed = {tf_lr: curr_lr, tf_meta_lr: curr_meta_lr}
        for j in range(n_tasks):
            comp = vae_comps[j]
            feed[comp.x] =  raw_obs_list[j]
        ...
        if i%3 != 1: # fixing
            (kl2vae, meta_cost, rnn_cost, vae_cost, transpose_cost, rnn_logstd, rnn_meta_logstd, vae_logstd, _) = sess.run([kl2vae_mean, rnn_meta_total_loss,
                                                             rnn_total_loss, vae_total_loss, transpose_loss,
                                                              rnn_mean_logstd,  rnn_mean_meta_logstd, vae_mean_logstd ,vae_all_op], feed)
        else: # breaking
            (kl2vae, rnn_cost, vae_cost, transpose_cost, rnn_logstd, vae_logstd, _, _) = sess.run([kl2vae_mean,
                                                             rnn_total_loss, vae_total_loss, transpose_loss,
                                                              rnn_mean_logstd,  vae_mean_logstd,
                                                              rnn_meta_op, vae_meta_op], feed)
        
        if (i % 30 == 0 and step > 0):
            end = time.time()
            time_taken = end - start
            start = time.time()
            #log_value("training loss", meta_cost, int(step // 20))
            output_log = "step: %d, lr: %.6f, kl2vae:%.2f, meta cost: %.2f, vae cost: %.2f, " \
                         "rnn cost: %.2f, transpose cost: %.2f, rstd:%.4f, mrstd:%.4f, vstd:%.4f" % \
                         (step, curr_lr, kl2vae, meta_cost, vae_cost, rnn_cost, transpose_cost, rnn_logstd, rnn_meta_logstd, vae_logstd)
            #output_log = "step: %d, lr: %.6f, kl2vae:%.2f, vae cost: %.2f, " \
            #             "rnn cost: %.2f, transpose cost: %.2f, rstd:%.4f, vstd:%.4f" % \
            #             (step, curr_lr, kl2vae, vae_cost, rnn_cost, transpose_cost, rnn_logstd, vae_logstd)
            print(output_log)
...
def main():
    ...
    parser.add_argument('--meta-lr', type=float, default=0.001, help="minimum of learning rate")
    parser.add_argument('--min-meta-lr', type=float, default=0.00001, help="minimum of learning rate")
    parser.add_argument('--meta-decay', type=float, default=0.998, help="decay of learning rate")
    # to load
