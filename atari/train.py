...
### MPI related code
comm = MPI.COMM_WORLD # To program multi-processing.
rank = comm.Get_rank()
...
def master():
  ...
  filename = filebase+'.json' # in charge of logging.
  ...
  while True:
    ...
    solutions = es.ask() # soluution ? or update algorithm ?

    if antithetic: # i don't understand this.
    ...
    # sync between the master and slaves
    send_packets_to_slaves(packet_list)
    reward_list_total = receive_packets_from_slaves()
    ...
    # initialized in the initialize_settings.
    es.tell(reward_list)
...
def main(args):
  ...
  optimizer = args.optimizer # PG or ES ( Many different kinds of evolution strategy )
  num_episode = args.num_episode # ( number of episodes )
