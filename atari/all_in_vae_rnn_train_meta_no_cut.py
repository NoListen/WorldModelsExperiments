...
def build_rnn_with_vae(vae, rnn, rnn_lv_dict, comp, z_size, seq_len, batch_size, rnn_lr, kl_tolerance=0.5):
    ...
    return rnn_comp, grads

def build_meta_rnn_with_vae(vae, rnn, rnn_lv_dict, comp, z_size, seq_len, batch_size, rnn_lr, kl_tolerance=0.5):
...
def learn(sess, n_tasks, z_size, data_dir, num_steps, max_seq_len,
    ...
    for i in range(n_tasks):
        ...
        rnn_vcomp, grads = build_rnn_with_vae(vae, rnn, 
    ...
    for i in range(n_tasks):
        tmp_rnn_lv_dict = copy.copy(rnn_lv_dict)
        # Mix the gradients.
        # Then, VAE learned from the meta loss.
        for k in rnn_lv_dict.keys():
            tmp_rnn_lv_dict[k] = tmp_rnn_lv_dict[k] - tf_r_lr * rnn_grads[n_tasks-i-1][k]

        rnn_meta_vcomp = build_meta_rnn_with_vae(vae, rnn, 
                                                  tmp_rnn_lv_dict, comp, z_size,
                                                  seq_len, batch_size_per_task, tf_r_lr, kl_tolerance)
    ...
    for i in range(n_tasks):
        ...
        if fc_limit:
          gvs = vae_rnn_opt.compute_gradients(rnn_losses[i]+rnn_meta_losses[i], comp.fc_var_list)
        else:
          gvs = vae_rnn_opt.compute_gradients(rnn_losses[i]+rnn_meta_losses[i], comp.var_list)
    ...
    for i in range(joint_num_steps):
        ...
        for _ in range(20):
          ...
          raw_meta_obs_list, raw_meta_a_list = dm.random_batch(batch_size_per_task)
          raw_meta_obs_list = [obs.reshape((-1,) + obs.shape[2:]) for obs in raw_meta_obs_list]
          ...
          for j in range(n_tasks):
              ...
              feed[comp.mx] =  raw_meta_obs_list[j]
              feed[comp.ma] =  raw_meta_a_list[j][:, :-1, :]
        ...
        if (i%1 == 0):
            output_log = "step: %d, lr: %.6f, kl2vae:%.2f, vae cost: %.2f, " \
                         ...
                         (step, curr_lr, kl2vae, vae_cost, rnn_cost, transform_cost, ptransform_cost, rnn_logstd, vae_logstd)
            print(output_log)
