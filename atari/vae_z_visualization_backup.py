...
def learn(sess, n_tasks, z_size, data_dir, num_steps, max_seq_len,
    ...
    tys = tf.split(ty, 2, axis=2)

    #cy = tf.concat([tf.stop_gradient(tys[0]), tys[1]], axis=2)
    controlled_gv = tf.gradients(tys[1], comp.z)


    #oy = tf.concat([tys[0], tf.stop_gradient(tys[1])], axis=2)
    opponent_gv = tf.gradients(tys[0], comp.z)
    total_grads = tf.gradients(tf.reduce_sum(y, axis=[1,2,3]), comp.z)
    ...
    for i in range(1):
      ...
      for _ in range(1):
          ...
          (c_gv, op_gv, mu, logstd) = sess.run([controlled_gv, opponent_gv, comp.mu, comp.logstd], feed)
      ...
      #print(output_log)
      #print(len(op_gv), op_gv[0].shape)
      #print(np.mean(op_gv[0], axis=0))
      #print(np.mean(c_gv[0], axis=0))
      print(np.mean(np.abs(mu), axis=0))
      print(np.mean(logstd, axis=0))
      #print((np.mean(np.abs(op_gv[0]), axis=0)>0.1).astype(np.int))
      ids =(np.mean(np.abs(c_gv[0]), axis=0)>0.1).astype(np.int)

      #print((np.mean(np.abs(c_gv[0]), axis=0)>0.1).astype(np.int))
      #print(np.mean(np.abs(c_gv[0]-op_gv[0]), axis=0))
      #print(np.mean((c_gv[0]-op_gv[0]), axis=0))

      #print(ids*np.mean(op_gv[0], axis=0))
      #print(ids*np.mean(c_gv[0], axis=0))
