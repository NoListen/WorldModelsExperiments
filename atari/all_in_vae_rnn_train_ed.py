...
def build_vae(name, vae, na, z_size, seq_len, vae_lr, kl_tolerance, fc_limit=False):
    ...
    vae_enc_var_list = vae.get_enc_variables()
    vae_dec_var_list = vae.get_dec_variables()
    # no decay
    ...
    scale_gvs = []
    for grad, var in vae_grads:
      if 'dec' in var.name:
        #scale_gvs.append((grad, var))
        scale_gvs.append((grad*0.5, var))
      else:
        scale_gvs.append((grad, var))
    vae_train_op = vae_opt.apply_gradients(scale_gvs, name=name+'train_op')
...
def learn(sess, n_tasks, z_size, data_dir, num_steps, max_seq_len,
    ...
    for grad, var in gvs:
      if 'dec' in var.name:
        ...
        scale_gvs.append((grad, var)) 
    vae_meta_op = vae_meta_opt.apply_gradients(scale_gvs, name='vae_meta_op')
