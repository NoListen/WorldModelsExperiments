...
def learn(sess, n_tasks, z_size, data_dir, num_steps, max_seq_len,
    ...
    for i in range(joint_num_steps):
        ...
        for _ in range(3):
          ...
          (kl2vae, rnn_cost, rnn_meta_cost, vae_cost, transpose_cost, ptranspose_cost, rnn_logstd, rnn_meta_logstd, vae_logstd, _, _) = sess.run([kl2vae_mean,
                                                             rnn_total_loss, rnn_meta_total_loss,  vae_total_loss, transpose_loss, ptranspose_loss,
                                                              rnn_mean_logstd, rnn_mean_meta_logstd, vae_mean_logstd,
        ...
        if (i % 10 == 0): #and step > 0):
            ...
            output_log = "step: %d, lr: %.6f, kl2vae:%.2f, vae cost: %.2f, meta cost: %.2f" \
                         "rnn cost: %.2f, t cost: %.2f, pt cost: %.2f, rstd:%.2f, mstd:%.2f, vstd:%.2f" % \
                         (step, curr_lr, kl2vae, vae_cost, rnn_meta_cost, rnn_cost, transpose_cost, ptranspose_cost, 
                            rnn_logstd, rnn_meta_logstd, vae_logstd)
            print(output_log)
