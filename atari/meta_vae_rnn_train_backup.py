...
VAE_COMP = namedtuple('VAE_COMP', ['a', 'x', 'y', 'z', 'ma', 'mx', 'my', 'mz', 'r_loss', 'kl_loss',
...
def build_vae(name, vae, na, z_size, seq_len, vae_lr, kl_tolerance):
    ...
    mz = vae.build_encoder(mx, reuse=True)
    ...
    vae_comp = VAE_COMP(a=a, x=x, z=z, y=y, ma=ma, mx=mx, mz=mz, my=my,
...
def learn(sess, n_tasks, z_size, data_dir, num_steps, max_seq_len,
    ...
    for i in range(n_tasks):
        ...
        rnn_loss, rnn_meta_loss = build_rnn_with_vae(rnn, tmp_rnn_lv_dict, comp, z_size,
...
def main():
    ...
    parser.add_argument('--num-steps', default=8000, help="number of training iterations")
