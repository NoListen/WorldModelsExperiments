...
VAE_COMP = namedtuple('VAE_COMP', ['a', 'x', 'y', 'z', 'mu', 'logstd', 'ma', 'mx', 'my', 'mz', 'mmu', 'mlogstd', 
                                    'r_loss', 'kl_loss', 'loss', 'var_list', 'fc_var_list', 'train_op', 'enc_op', 'dec_op'])
...
def build_vae(name, vae, na, z_size, seq_len, vae_lr, kl_tolerance, fc_limit=False):
    ...
    vae_enc_opt = tf.train.AdamOptimizer(vae_lr)
    vae_dec_opt = tf.train.AdamOptimizer(0.5*vae_lr)
    ...
    vae_enc_op = vae_enc_opt.apply_gradients([(grad, var) for grad, var in vae_grads if 'enc' in var.name], name=name+"enc_op")
    vae_dec_op = vae_dec_opt.apply_gradients([(grad, var) for grad, var in vae_grads if 'dec' in var.name], name=name+"dec_op")
    ...
    vae_comp = VAE_COMP(a=a, x=x, z=z, y=y, mu=mu, logstd=logstd, ma=ma, mx=mx, mz=mz, my=my,
                        ...
                        var_list=vae_var_list, fc_var_list=vae_fc_var_list, enc_op = vae_enc_op,
                        dec_op=vae_dec_op, train_op=vae_train_op)
    return vae_comp
...
def learn(sess, n_tasks, z_size, data_dir, num_steps, max_seq_len,
    ...
    vae_all_enc_op = tf.group([comp.enc_op for comp in vae_comps])
    vae_all_dec_op = tf.group([comp.dec_op for comp in vae_comps])
    ...
    #dec_gvs = [(grad, var) for grad, var in gvs if 'dec' in var.name]
