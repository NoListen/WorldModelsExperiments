...
class DatasetConcatWrapper(object):
    def __init__(self, dataset):
        self.dataset = dataset
...
def learn(sess, n_tasks, z_size, data_dir, num_steps, max_seq_len,
    ...
    datasets = [dataset1, DatasetVerticalConcatWrapper(dataset2)]
    ...
    ty = tf.split(ty, 2, axis=1)
    tty = tf.concat(ty[::-1], axis=1)
    ...
    py = tf.split(py, 2,axis=1)
    py = tf.concat(py[::-1], axis=1)
