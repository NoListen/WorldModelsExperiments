...
RNN_COMP_WITH_OPT = namedtuple('RNN_COMP', ['z_input', 'a', 'mean', 'logstd', 'var_list'])
RNN_COMP_WITH_VAE = namedtuple('RNN_COMP_WITH_VAE', ['a',  'mean', 'logstd', 'loss'])
...
def build_rnn(name, rnn, na, z_size, batch_size, seq_len):
    ...
    pz, out_mean, out_logstd = rnn.build_model(input_x)

    rnn_var_list = rnn.get_variables()
    rnn_comp = RNN_COMP_WITH_OPT(a=a, z_input=rnn_z,
...
def build_rnn_with_vae(vae, rnn, rnn_lv_dict, comp, z_size, seq_len, batch_size, rnn_lr, kl_tolerance=0.5):
    rnn_loss, mu, logstd = rnn_with_vae(vae, rnn, comp.x, comp.z, comp.a, rnn_lv_dict,
    ...
    kl2vae = tf.reduce_mean(get_kl_lossfunc(mu, logstd, target_mu, target_logstd))

    return rnn_loss, mu, logstd, target_mu, target_logstd, kl2vae
...
def learn(sess, n_tasks, z_size, data_dir, num_steps, max_seq_len,
    ...
    for i in range(n_tasks):
        ...
        rnn_loss, mu, logstd, target_mu, target_logstd, kl2vae = build_rnn_with_vae(vae, rnn,
                                               tmp_rnn_lv_dict, comp, z_size,
                                               seq_len, batch_size_per_task, tf_lr, kl_tolerance)

        rnn_mus.append(mu)
        rnn_logstds.append(logstd)
        rnn_tmus.append(target_mu)
        rnn_tlogstds.append(target_logstd)
    ...
    for i, comp in enumerate(vae_comps):
        loadFromFlat(comp.var_list, model_dir+"/final_vae%i.p" % i)

    if os.path.exists(model_dir+'/final_rnn.p'):
        loadFromFlat(rnn_comp.var_list, model_dir+'/final_rnn.p')
    ...
    def get_kl(mean, logstd, target_mean, target_logstd):
        kl = logstd - target_logstd + (np.exp(2*target_logstd)+np.square(target_mean-mean))/2/np.exp(2*logstd) - 0.5
        return np.sum(kl, axis=1)
    (kl2vae, rnn_cost, vae_cost, transpose_cost, rnn_logstd, vae_logstd) = sess.run([kl2vae_mean,
                                                             rnn_total_loss, vae_total_loss, transpose_loss,
                                                              rnn_mean_logstd,  vae_mean_logstd], feed)
    print(kl2vae, rnn_cost, vae_cost, transpose_cost, rnn_logstd, vae_logstd)
    id = 0
    (mu, logstd, rnn_mu, rnn_logstd) = sess.run([rnn_mus[0], rnn_logstds[0], rnn_tmus[0], rnn_tlogstds[0]], feed)
    print(mu[0], rnn_mu[0])
    print(logstd[0], rnn_logstd[0])
    for i in range(len(mu)):
      kl = get_kl(mu[i:i+1], logstd[i:i+1], rnn_mu[i:i+1], rnn_logstd[i:i+1])
      if kl > 5000:
        print(kl)
        print(mu[i:i+1], rnn_mu[i:i+1])
        print(logstd[i:i+1], rnn_logstd[i:i+1])
        print(np.sum((mu[i:i+1]- rnn_mu[i:i+1])**2))
        print(np.sum((logstd[i:i+1]- rnn_logstd[i:i+1])**2))
        break
...
def main():
    ...
    parser.add_argument('--kl-tolerance', type=float, default=0.1, help="kl tolerance")
